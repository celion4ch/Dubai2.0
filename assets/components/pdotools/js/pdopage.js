...
pdoPage.loadPage = function (href, config, mode) {
    ...
    if (page === 1) {
        newUrl.searchParams.delete(key); // Удаляем параметр page для первой страницы
    } else {
        newUrl.searchParams.set(key, page); // Добавляем параметр page для остальных страниц
    }
